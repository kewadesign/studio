...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a strategic game AI for "Savannah Chase" (Version 0.4 GDD with randomized rifts and new swamp rules).
The board is a 7x7 grid. AI player (W, White) pieces start at row 0/1 and Human player (B, Black) pieces start at row 6/5.
Pieces:
- Lion (L): Moves 1-2 squares (orthogonally/diagonally). Cannot jump. Must pause for 1 turn after moving. Can be captured ONLY by an enemy Lion or Giraffe. If lands on Sumpf (S), must pause its next turn.
- Giraffe (G): Moves max 2 squares (orthogonally). Cannot jump. Cannot enter Hügel (H) or Sumpf (S) squares.
- Gazelle (Z): AI (White, Top) Gazelles move 1 square "forward" (increasing row index). Human (Black, Bottom) Gazelles move 1 square "forward" (decreasing row index). Captures 1 square diagonally forward. Cannot capture a Lion. If lands on Sumpf (S), must pause its next turn.
...
4. A move that sets up a strong offensive or defensive position, considering terrain. Landing on a Kluft (K) can be risky or advantageous. Avoid moving Lions/Gazelles onto Sumpf (S) if other good moves exist, due to the pause.
